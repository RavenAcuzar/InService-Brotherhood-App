    ...
    this.storage.get(LOGGED_IN_KEY).then(isLogged => {
      this.storage.get(USER_DATA_KEY).then(userdetails => {
        if (isLogged) {
          ...
          this.subs = this.appSvc.getRequest(this.eventURL, userdetails.IRID).then(requestData => {
            if (requestData) {
              this.disabled = true;
              this.disabled2 = true;
              if (requestData.status == 'Approved' || requestData.status == 'Paid' || requestData.status == 'Onsite Payment') {
                var title;
                var mess;
                var btn;
                ...
                if(requestData.status=='Onsite Payment'){
                  ...
                  mess="Prepare your payment onsite. Click 'Proceed' to view your things to bring and map of the event.";
                  btn="Proceed";
                }
                  ...
                  buttons: [
                    {
                      text: btn,
                      handler: () => {
                        this.goToPaymentInstructions(requestData.orderID, requestData.price, requestData.paymentStatus, requestData.RequestType);
                      }
                    },
                    {
                      text: 'Cancel',
  ...
  submitRequest(eventID: string, reqType: string, eventName: string, eventDate: string, eventVenue: string) {
    ...
    */
    this.storage.get(USER_DATA_KEY).then(data => {
      if (data.city_address == '' || data.country_address == '' || data.gender == '' || data.h_address == '' || data.mobile_no == '' || data.state_address == '' || data.birth == '' || data.zip_code == '' || data.tel_no == '') {
            ...
            req = this.http.post('http://bt.the-v.net/service/api.aspx', body, this.options)
              .subscribe(res => {
                ...
                if (resp[0].length == 0) {
                  ...
                  this.appSvc.submitUserRequest(eventID, user, reqType, eventName, eventDate, eventVenue, this.userEmail);
                }
                else {
                  let youralert = this.alertCtrl.create({
